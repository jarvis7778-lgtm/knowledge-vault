...
- **Obsidian vault root**: `/path/to/your/obsidian-vault` (示例占位符)
- **All knowledge content lives under**: `knowledge/`
  - `knowledge/Inbox/`
...
Hard rule: **do not create or use a nested `vault/` folder**. 这里的“vault”仅指你自己的 Obsidian vault 根目录。
...
2) **Ask for `direction/方向` only if missing**
   - If user did not provide a direction, **default `direction: auto`** (do NOT default to Inbox).
   - `direction: auto` MUST be **resolved into a concrete folder** (e.g. `code`, `co-in`, `Inbox`) by the ingester before writing.
